...
def _compute_wclip_wunclip_and_act_frac(
    ...
    # Sequence-level advantage proxy
    A_hat = torch.sum(advantages * response_mask, dim=-1) / torch.sum(response_mask, dim=-1)  # (B,)
    ...
    ratio = torch.exp(neg_approx_kl)  # (B, T)

    a_pos = (A_hat >= 0).unsqueeze(-1)  # (B, 1)
    ind = torch.where(
        a_pos,
        ratio <= (1.0 + cliprange_high),
        ratio >= (1.0 - cliprange_low),
    )

    ind_f = ind.to(dtype=response_mask.dtype)
    rho_ind_unclip_mean = torch.sum(ratio * response_mask, dim=-1) / torch.sum(response_mask, dim=-1)
    rho_ind_clip_mean = torch.sum(ratio * ind_f * response_mask, dim=-1) / torch.sum(response_mask, dim=-1)

    w_unclip = A_hat * rho_ind_unclip_mean
    w_clip = A_hat * rho_ind_clip_mean

    # fraction of unclipped tokens among batch
    act_frac = torch.sum(ind_f * response_mask) / torch.sum(response_mask)
    return w_clip, w_unclip, act_frac
...
@register_policy_loss("hack")
def compute_policy_loss_hack(
    old_log_prob: torch.Tensor,
    log_prob: torch.Tensor,
    advantages: torch.Tensor,
    response_mask: torch.Tensor,
    ...
    **kwargs: Any,
    ...
    """
    A debug/analysis loss mode:
    - Computes the standard PPO clipped objective (same as vanilla)
    - Additionally computes:
        * E[|w_clip - w_unclip|]
        * Cov(u_m, w_clip) within each prompt-group (uid), averaged across prompts

    Returns:
      (pg_loss, pg_clipfrac, ppo_kl, pg_clipfrac_lower, extra_metrics)
    """
    assert config is not None
    assert not isinstance(config, AlgoConfig)

    enable = bool(kwargs["enable"])
    per_objective_scores = kwargs["per_objective_scores"]
    uids = kwargs["uid"]
    obj_names = kwargs["objective_names"]
    ...
    # No dual-clip: use standard PPO clipped objective for all tokens.
    ...
    if enable:
        with torch.no_grad():
            w_clip, w_unclip, act_frac = _compute_wclip_wunclip_and_act_frac(
            ...
            w_diff = w_clip - w_unclip
            extra["clip/wclip/dist_abs"] = float(w_diff.abs().mean().item())
            extra["clip/wclip/dist"] = float(w_diff.mean().item())
            extra["clip/wclip/act_frac"] = float(act_frac.item())

            u_mat = per_objective_scores.detach().to("cpu").numpy()  # (B, M)
            w_vec = w_clip.detach().to("cpu").numpy()  # (B,)
            uid_list = uids.tolist()

            # if this goes ~0, wclip_cov will naturally go ~0.
            extra["clip/wclip/std"] = float(np.std(w_vec))

            assert len(obj_names) == u_mat.shape[1], (
                f"len(objective_names)={len(obj_names)} must match per_objective_scores.shape[1]={u_mat.shape[1]}"
            )
            ...
            cov_sum = np.zeros((u_mat.shape[1],), dtype=np.float64)
            n_groups = 0
            for idxs in uid2idxs.values():
                ...
                w_c = w_g - w_g.mean()
                u_g = u_mat[idxs, :]
                u_c = u_g - u_g.mean(axis=0, keepdims=True)  # (k, M)
                cov_sum += np.mean(u_c * w_c[:, None], axis=0)  # (M,)
                n_groups += 1

            # Useful for interpreting NaNs on micro-batches (e.g., when all groups have <2 samples).
            extra["clip/wclip_cov/n_groups"] = float(n_groups)

            if n_groups > 0:
                cov_mean = cov_sum / float(n_groups)
                for j, name in enumerate(obj_names):
                    extra[f"clip/wclip_cov/{name}"] = float(cov_mean[j])
            else:
                for name in obj_names:
                    extra[f"clip/wclip_cov/{name}"] = -100.0
