...
def get_single_reward_fn(file_path, function_name):
...
def get_custom_reward_fn(config: DictConfig) -> Optional[RawRewardFn]:
    ...
    if isinstance(file_path, ListConfig):
        return {path_to_name(path): get_single_reward_fn(path, function_name) for path in file_path}
    else:
        # return partial(_call_with_kwargs, get_single_reward_fn(file_path, function_name), reward_kwargs)
        return get_single_reward_fn(file_path, function_name)
...
def load_reward_manager(
    ...
    # multiobjective: MultiObjectiveRewardManager
    ...
    # NOTE: will not be used for multiobjective reward manager
