...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def _build_model_optimizer(
        ...
        if role == "actor" and optim_config is not None:
            from verl.utils.torch_functional import (
                ...
                get_polynomial_schedule_with_warmup,
            )
            ...
            if warmup_style == "constant":
                ...
                actor_lr_scheduler = get_exponential_schedule_with_warmup(
                    optimizer=actor_optimizer, 
                    num_warmup_steps=num_warmup_steps, 
                    gemma=gamma,
                )
            elif warmup_style == "polynomial":
                actor_lr_scheduler = get_polynomial_schedule_with_warmup(
                    optimizer=actor_optimizer,
                    num_warmup_steps=num_warmup_steps,
                    num_training_steps=total_steps,
                    power=power,
    ...
    def init_model(self):
        ...
        if self._is_actor:
            ...
            actor_cfg = self.actor_model_config

            head_dim = getattr(actor_cfg, "head_dim", None)
            hidden_size = getattr(actor_cfg, "hidden_size", None)
            num_heads = getattr(actor_cfg, "num_attention_heads", None)
            if (
                head_dim is None
                ...
                actor_cfg.head_dim = int(hidden_size) // int(num_heads)
    ...
    @register(dispatch_mode=make_nd_compute_dataproto_dispatch_fn(mesh_name="actor"))
    @DistProfiler.annotate(color="red", role="actor_update_covariance")
    def update_actor_covariance(self, data: DataProto):
        assert self._is_actor
        if self._is_offload_param:
            load_fsdp_model_to_gpu(self.actor_module_fsdp)
        if self._is_offload_optimizer:
        ...
        with self.ulysses_sharding_manager:
            ...
            with Timer(name="update_policy_covariance", logger=None) as timer:
                metrics = self.actor.update_policy_covariance(data=data)
            delta_time = timer.last
        ...
        if self._is_offload_param:
            offload_fsdp_model_to_cpu(self.actor_module_fsdp)
            log_gpu_memory_usage("After offload actor model during update_actor_covariance", logger=logger)
        if self._is_offload_optimizer:
            offload_fsdp_optimizer(optimizer=self.actor_optimizer)
            log_gpu_memory_usage("After offload actor optimizer during update_actor_covariance", logger=logger)

        return output

    @register(dispatch_mode=make_nd_compute_dataproto_dispatch_fn(mesh_name="actor"))
    @DistProfiler.annotate(color="red", role="actor_update_mgda")
    def update_actor_mgda(self, data: DataProto):
        assert self._is_actor
        if self._is_offload_param:
            load_fsdp_model_to_gpu(self.actor_module_fsdp)
        if self._is_offload_optimizer:
        ...
        with self.ulysses_sharding_manager:
            ...
            with Timer(name="update_policy_mgda", logger=None) as timer:
                metrics = self.actor.update_policy_mgda(data=data)
            delta_time = timer.last
            global_num_tokens = data.meta_info["global_token_num"]
            estimated_flops, promised_flops = self.flops_counter.estimate_flops(global_num_tokens, delta_time)
            metrics["perf/mfu/actor"] = (
        ...
        if self._is_offload_param:
            offload_fsdp_model_to_cpu(self.actor_module_fsdp)
            log_gpu_memory_usage("After offload actor model during update_actor_mgda", logger=logger)
        if self._is_offload_optimizer:
            offload_fsdp_optimizer(optimizer=self.actor_optimizer)
            log_gpu_memory_usage("After offload actor optimizer during update_actor_mgda", logger=logger)

        return output

    @register(dispatch_mode=make_nd_compute_dataproto_dispatch_fn(mesh_name="actor"))
    @DistProfiler.annotate(color="red", role="actor_update_nash_mtl")
    def update_actor_nash_mtl(self, data: DataProto):
        assert self._is_actor
        if self._is_offload_param:
            load_fsdp_model_to_gpu(self.actor_module_fsdp)
        if self._is_offload_optimizer:
        ...
        with self.ulysses_sharding_manager:
            ...
            with Timer(name="update_policy_nash_mtl", logger=None) as timer:
                metrics = self.actor.update_policy_nash_mtl(data=data)
            delta_time = timer.last
            global_num_tokens = data.meta_info["global_token_num"]
            estimated_flops, promised_flops = self.flops_counter.estimate_flops(global_num_tokens, delta_time)
            metrics["perf/mfu/actor"] = (
        ...
        if self._is_offload_param:
            offload_fsdp_model_to_cpu(self.actor_module_fsdp)
            log_gpu_memory_usage("After offload actor model during update_actor_nash_mtl", logger=logger)
        if self._is_offload_optimizer:
            offload_fsdp_optimizer(optimizer=self.actor_optimizer)
            log_gpu_memory_usage("After offload actor optimizer during update_actor_nash_mtl", logger=logger)

        return output

    @register(dispatch_mode=make_nd_compute_dataproto_dispatch_fn(mesh_name="actor"))
    @DistProfiler.annotate(color="red", role="actor_update_famo")
    def update_actor_famo(self, data: DataProto):
        assert self._is_actor
        if self._is_offload_param:
            load_fsdp_model_to_gpu(self.actor_module_fsdp)
        if self._is_offload_optimizer:
        ...
        with self.ulysses_sharding_manager:
            ...
            with Timer(name="update_policy_famo", logger=None) as timer:
                metrics = self.actor.update_policy_famo(data=data)
            delta_time = timer.last
            global_num_tokens = data.meta_info["global_token_num"]
            estimated_flops, promised_flops = self.flops_counter.estimate_flops(global_num_tokens, delta_time)
            metrics["perf/mfu/actor"] = (
        ...
        if self._is_offload_param:
            offload_fsdp_model_to_cpu(self.actor_module_fsdp)
            log_gpu_memory_usage("After offload actor model during update_actor_famo", logger=logger)
        if self._is_offload_optimizer:
            offload_fsdp_optimizer(optimizer=self.actor_optimizer)
            log_gpu_memory_usage("After offload actor optimizer during update_actor_famo", logger=logger)

        return output
    
    @register(dispatch_mode=make_nd_compute_dataproto_dispatch_fn(mesh_name="actor"))
    @DistProfiler.annotate(color="red", role="actor_update_gradnorm")
    def update_actor_gradnorm(self, data: DataProto):
        assert self._is_actor
        if self._is_offload_param:
            load_fsdp_model_to_gpu(self.actor_module_fsdp)
        if self._is_offload_optimizer:
        ...
        with self.ulysses_sharding_manager:
            ...
            with Timer(name="update_policy_gradnorm", logger=None) as timer:
                metrics = self.actor.update_policy_gradnorm(data=data)
            delta_time = timer.last
            global_num_tokens = data.meta_info["global_token_num"]
            estimated_flops, promised_flops = self.flops_counter.estimate_flops(global_num_tokens, delta_time)
            metrics["perf/mfu/actor"] = (
        ...
        if self._is_offload_param:
            offload_fsdp_model_to_cpu(self.actor_module_fsdp)
            log_gpu_memory_usage("After offload actor model during update_actor_gradnorm", logger=logger)
        if self._is_offload_optimizer:
            offload_fsdp_optimizer(optimizer=self.actor_optimizer)
            log_gpu_memory_usage("After offload actor optimizer during update_actor_gradnorm", logger=logger)

        return output

    @register(dispatch_mode=make_nd_compute_dataproto_dispatch_fn(mesh_name="actor"))
    @DistProfiler.annotate(color="red", role="actor_update_pama")
    def update_actor_pama(self, data: DataProto):
        assert self._is_actor
        if self._is_offload_param:
            load_fsdp_model_to_gpu(self.actor_module_fsdp)
        if self._is_offload_optimizer:
        ...
        with self.ulysses_sharding_manager:
            ...
            with Timer(name="update_policy_pama", logger=None) as timer:
                metrics = self.actor.update_policy_pama(data=data)
            delta_time = timer.last
            global_num_tokens = data.meta_info["global_token_num"]
            estimated_flops, promised_flops = self.flops_counter.estimate_flops(global_num_tokens, delta_time)
            metrics["perf/mfu/actor"] = (
        ...
        if self._is_offload_param:
            offload_fsdp_model_to_cpu(self.actor_module_fsdp)
            log_gpu_memory_usage("After offload actor model during update_actor_pama", logger=logger)
        if self._is_offload_optimizer:
            offload_fsdp_optimizer(optimizer=self.actor_optimizer)
            log_gpu_memory_usage("After offload actor optimizer during update_actor_pama", logger=logger)

        return output

    @register(dispatch_mode=make_nd_compute_dataproto_dispatch_fn(mesh_name="actor"))
    @DistProfiler.annotate(color="red", role="actor_update_lagrangian")
    def update_actor_lagrangian(self, data: DataProto):
        """Update actor parameters using stochastic primal-dual Lagrangian weights."""
        assert self._is_actor
        if self._is_offload_param:
            load_fsdp_model_to_gpu(self.actor_module_fsdp)
        if self._is_offload_optimizer:
        ...
        with self.ulysses_sharding_manager:
            ...
            with Timer(name="update_policy_lagrangian", logger=None) as timer:
                metrics = self.actor.update_policy_lagrangian(data=data)
            delta_time = timer.last
            global_num_tokens = data.meta_info["global_token_num"]
            estimated_flops, promised_flops = self.flops_counter.estimate_flops(global_num_tokens, delta_time)
            metrics["perf/mfu/actor"] = (
        ...
        if self._is_offload_param:
            offload_fsdp_model_to_cpu(self.actor_module_fsdp)
            log_gpu_memory_usage("After offload actor model during update_actor_lagrangian", logger=logger)
        if self._is_offload_optimizer:
            offload_fsdp_optimizer(optimizer=self.actor_optimizer)
            log_gpu_memory_usage("After offload actor optimizer during update_actor_lagrangian", logger=logger)

        return output

    @register(dispatch_mode=make_nd_compute_dataproto_dispatch_fn(mesh_name="actor"))
    @DistProfiler.annotate(color="red", role="actor_compute_gradients")
    def compute_gradients(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            ...
            flat_grad_concat = self.actor.compute_gradients(
                data=data,
                target_layer_ids=self.config.actor.target_layer_ids
            )

            tensors = {"flat_grad_concat": flat_grad_concat}
    ...
    @register(dispatch_mode=make_nd_compute_dataproto_dispatch_fn(mesh_name="actor"))
    @DistProfiler.annotate(color="red", role="actor_apply_gradients")
    def apply_gradients(self, data: DataProto):
        """
        Apply a flat gradient vector to parameters and step optimizer.
        """
        assert self._is_actor, "Apply gradients requires actor role"
        if self._is_offload_param:
            load_fsdp_model_to_gpu(self.actor_module_fsdp)
        if self._is_offload_optimizer:
            load_fsdp_optimizer(self.actor_optimizer, device_id=get_torch_device().current_device())
        ...
        with self.ulysses_sharding_manager:
            data = self.ulysses_sharding_manager.preprocess_data(data=data)
            flat_grad_concat = data.batch["flat_grad_concat"]
            num_params_per_layer = data.meta_info["num_params_per_layer"]
            metrics = self.actor.apply_flat_gradients(
                flat_grad_concat=flat_grad_concat,
        ...
        if self._is_offload_param:
            offload_fsdp_model_to_cpu(self.actor_module_fsdp)
            log_gpu_memory_usage("After off-loading actor model during apply_gradients", logger=logger)
        if self._is_offload_optimizer:
            offload_fsdp_optimizer(self.actor_optimizer)
            log_gpu_memory_usage("After off-loading actor optimiser during apply_gradients", logger=logger)
...
class CriticWorker(Worker, DistProfilerExtension):
    ...
    def init_model(self):
        ...
        critic_cfg = self.critic_model_config

        head_dim = getattr(critic_cfg, "head_dim", None)
        hidden_size = getattr(critic_cfg, "hidden_size", None)
        num_heads = getattr(critic_cfg, "num_attention_heads", None)
        if (
            head_dim is None
            and hidden_size is not None
            and num_heads is not None
            ...
            critic_cfg.head_dim = int(hidden_size) // int(num_heads)
