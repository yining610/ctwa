...
PROJECT_NAME="smooth_tchebycheff-reinforce-math500"
...
REF_MODE="running_max"
OMD_LR=0.3
SMOOTH_TEMPERATURE=1.0
SMOOTH_EPS=1e-8
SMOOTH_INIT="preference" # options: uniform, preference
...
python3 -m verl.trainer.main_ppo_multiobjective \
    ...
    algorithm.weighting_method=smooth_tchebycheff \
    algorithm.tchebycheff.ref_mode=$REF_MODE \
    algorithm.tchebycheff.reference_point=null \
    algorithm.smooth_tchebycheff.omd_lr=$OMD_LR \
    algorithm.smooth_tchebycheff.temperature=$SMOOTH_TEMPERATURE \
    algorithm.smooth_tchebycheff.eps=$SMOOTH_EPS \
    algorithm.smooth_tchebycheff.init=$SMOOTH_INIT \
